# WGCNA for pine reads
# cant include control pines for inoculated vs non-inoculated, not enough power
# can do seperate for treatment across pines
# and for ZnT vs ZnS on pine expression (and treatment) excluding control_pine reads
...
# make a df with desired samples 
countsall <- ref230_ %>%
  dplyr::select(matches("_bioassay"))

colnames(countsall)

# remove outliers previously identified
exclude_samples <- c(
  "X230_bioassay_0Zn_Rep4")
...
# sig pos corr - black, blue
# sig neg corr - green, red
...
redZnrank <- genes.pvals.Zn[moduleLabels=="red",] %>%
  arrange( ,rank, desc(rank)) %>%
  mutate(gene = sub(".*proteinId=([0-9]+).*", "\\1", gene))

blackZnrank <- genes.pvals.Zn[moduleLabels=="black",] %>%
  arrange( ,rank, desc(rank)) %>%
  mutate(gene = sub(".*proteinId=([0-9]+).*", "\\1", gene))

blueZnrank <- genes.pvals.Zn[moduleLabels=="blue",] %>%
  arrange( ,rank, desc(rank)) %>%
  mutate(gene = sub(".*proteinId=([0-9]+).*", "\\1", gene))
...
blackTolrank <- genes.pvals.Tol[moduleLabels=="black",] %>%
  arrange( ,rank, desc(rank)) %>%
  mutate(gene = sub(".*proteinId=([0-9]+).*", "\\1", gene))
...
# Remove leading/trailing quotes from gene names
names(gene_to_go_mapping) <- gsub('"', '', names(gene_to_go_mapping))

# Check
head(names(gene_to_go_mapping))
...
allResBP$Ontology <- "Biological Process"
allResCC$Ontology <- "Cellular Component"
allResMF$Ontology <- "Molecular Function"

bp <- allResBP[allResBP$weightFisher<0.05,]
cc <- allResCC[allResCC$weightFisher<0.05,]
mf <- allResMF[allResMF$weightFisher<0.05,]

table <- rbind(bp, cc, mf)
table
# write table for each module - change name!!
write.csv(table, "~/Desktop/Stomentosus_Zn_Transcriptomics/WGCNA_bioassay/bioassay_pine_yellowModule_GO.csv")
