...
countsall <- ref230_ %>%
  dplyr::select(grep("_bioassay", colnames(ref230_), value = T))
colnames(countsall)
...
# sig pos corr - blue, greenyellow, magenta
# sig neg corr - yellow, grey (pink is borderline)
turquoiseTolrank <- genes.pvals.Tol[moduleLabels=="turquoise",] %>%
  arrange( ,rank, desc(rank)) %>%
  mutate(gene = sub(".*proteinId=([0-9]+).*", "\\1", gene))
...
write.csv(table, "~/Desktop/Stomentosus_Zn_Transcriptomics/WGCNA_bioassay/bioassay_fungal_brownModule_GO.csv")
