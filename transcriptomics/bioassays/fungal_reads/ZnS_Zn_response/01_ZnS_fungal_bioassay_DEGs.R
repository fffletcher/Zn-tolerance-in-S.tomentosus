...
counts230all <- ref230_ %>%
  dplyr::select(grep("X220_bioassay", colnames(ref230_), value = T))
colnames(counts230all)

# remove outliers - identified previously
#bioassay230counts <- counts230all %>%
  #dplyr::select(!grep("220_bioassay_1Zn_Rep1", colnames(counts230all), value = T))
bioassay230counts <- counts230all
colnames(bioassay230counts)

# make col data files from samples in the df
sample_names <- colnames(bioassay230counts)
...
bioassay230counts[is.na(bioassay230counts)] <- 0
# Create DESeq object (note design only by treatment as they are all the same isolate)
ddsMat <- DESeqDataSetFromMatrix(countData = bioassay230counts,
                                 colData = col_data,
                                 design = ~ treatment)
...
# change gene_id col (protein ID)
res_shrunken_df <- res_shrunken_df %>%
  mutate(gene_id = sub(".*proteinId=([0-9]+).*", "\\1", gene_id))
cont_v_10_res_df <- cont_v_10_res_df %>%
  mutate(gene_id = sub(".*proteinId=([0-9]+).*", "\\1", gene_id))
cont_v_5_res_df <- cont_v_5_res_df %>%
  mutate(gene_id = sub(".*proteinId=([0-9]+).*", "\\1", gene_id))
cont_v_1_res_df <- cont_v_1_res_df %>%
  mutate(gene_id = sub(".*proteinId=([0-9]+).*", "\\1", gene_id))

# write these to csv and save for further anaysis
write.csv(cont_v_10_res_df, "~/Desktop/Stomentosus_Zn_Transcriptomics/Suitomtomento230_ref/Bioassay/220_cont_v_10.csv")
write.csv(cont_v_5_res_df, "~/Desktop/Stomentosus_Zn_Transcriptomics/Suitomtomento230_ref/Bioassay/220_cont_v_5.csv")
write.csv(cont_v_1_res_df, "~/Desktop/Stomentosus_Zn_Transcriptomics/Suitomtomento230_ref/Bioassay/220_cont_v_1.csv")
...
EnhancedVolcano(cont_v_10_res_df,
                ...
                gridlines.major = F,
                gridlines.minor = T) +
  ggplot2::coord_cartesian(xlim=c(-15, 15), ylim=c(0,50))
...
p <- ggplot() +
  ...
  geom_text(
    ...
    vjust = ifelse(reg_counts$count > 0, -1, 2),  # Above for up, below for down
