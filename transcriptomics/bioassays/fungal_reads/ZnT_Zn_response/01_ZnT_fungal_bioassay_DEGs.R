...
counts230all <- ref230_ %>%
  dplyr::select(grep("X230_bioassay", colnames(ref230_), value = T))
colnames(counts230all)

# remove outliers - identified previously
bioassay230counts <- counts230all %>%
  dplyr::select(!grep("230_bioassay_0Zn_Rep4", colnames(counts230all), value = T))
...
write.csv(cont_v_10_res_df, "~/Desktop/Stomentosus_Zn_Transcriptomics/Suitomtomento230_ref/Bioassay/230_cont_v_10.csv")
write.csv(cont_v_5_res_df, "~/Desktop/Stomentosus_Zn_Transcriptomics/Suitomtomento230_ref/Bioassay/230_cont_v_5.csv")
write.csv(cont_v_1_res_df, "~/Desktop/Stomentosus_Zn_Transcriptomics/Suitomtomento230_ref/Bioassay/230_cont_v_1.csv")
