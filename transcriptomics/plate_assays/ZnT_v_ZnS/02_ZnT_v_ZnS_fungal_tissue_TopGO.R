...
# Stomentosus220v230tissue_DEG_heatmaps
# otherwise necessary objects won't be present 

library(topGO)
library(dplyr)

# gene to GO mapping file has already been generated
...
go_df$weightFisher <- as.numeric(go_df$weightFisher)

# Filter for significant terms (adjusted p-value < 0.05)
sig_go_df <- go_df %>% filter(weightFisher < 0.05)
write.csv(sig_go_df, "~/Desktop/Stomentosus_Zn_Transcriptomics/Suitomtomento230_ref/Tissue/230v220_DEG_GO.csv")
