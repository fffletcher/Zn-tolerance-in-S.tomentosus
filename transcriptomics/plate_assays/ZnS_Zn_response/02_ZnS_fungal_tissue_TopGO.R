# run this in the same session as completed 
# Stomentosus220tissue_DEG_heatmaps
# otherwise necessary objects won't be present 
...
# 220 tissue assay RNA comparing 0 and 1 mM Zn
...
# use shrunken
de_genes <- cont_v_1_res_shrunken_df %>%
...
write.csv(sig_go_df, "~/Desktop/Stomentosus_Zn_Transcriptomics/Suitomtomento230_ref/Tissue/220_DEG_GO_1mM.csv")
...
sig_go_df$Category <- stringr::str_wrap(sig_go_df$Category, width = 5)
...
# volcano plot of genes in the term at 1 mM Zn
vol <- deg_goi[deg_goi$result_name=="treatment_1_vs_0",]

keyvals <- ifelse(
  vol$log2FoldChange < -1.5 & vol$padj <= 0.05, 'blue',
  ifelse(vol$log2FoldChange > 1.5 & vol$padj <= 0.05, 'red',
         'black'))
