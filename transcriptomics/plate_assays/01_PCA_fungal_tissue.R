...
# make a df for tissue only 
tissue_ref230 <- ref230_ %>%
  dplyr::select(grep("tissue", colnames(ref230_), value = T))

#remove outliers
# "X230_tissue_0Zn_Rep4", "X230_tissue_1Zn_Rep3", 
# "X220_tissue_0Zn_Rep1", "X220_tissue_1Zn_Rep1"

tissue_ref230_subset <- tissue_ref230 %>%
  dplyr::select(!grep("X230_tissue_0Zn_Rep4", colnames(tissue_ref230), value = T))
tissue_ref230_subset <- tissue_ref230_subset %>%
  dplyr::select(!grep("X230_tissue_1Zn_Rep3", colnames(tissue_ref230_subset), value = T))
tissue_ref230_subset <- tissue_ref230_subset %>%
  dplyr::select(!grep("X220_tissue_0Zn_Rep1", colnames(tissue_ref230_subset), value = T))
tissue_ref230_subset <- tissue_ref230_subset %>%
  dplyr::select(!grep("X220_tissue_1Zn_Rep1", colnames(tissue_ref230_subset), value = T))
tissue_ref230_subset <- tissue_ref230_subset %>%
   dplyr::select(!grep("_5Zn_", colnames(tissue_ref230_subset), value = T))

# make col data file
sample_names <- colnames(tissue_ref230_subset)
...
tissue_ref230_subset[is.na(tissue_ref230_subset)] <- 0
# Create DESeq object (note design only by treatment as they are all the same isolate)
ddsMat <- DESeqDataSetFromMatrix(countData = tissue_ref230_subset,
                                 colData = col_data,
...
plotPCA(vst, intgroup = c("treatment", "isolate")) +
  ...
  scale_color_manual(
    values = c("220" = "#E69F00", "230" = "#0072B2"),
...
# interaction effect
# whether treatment effects differ between isolates
# "Does the treatment effect depend on the isolate?"
adonis2(vegan ~ treatment * isolate, data = test, method = 'eu')
...
# only 230 isolate
i230_ref230 <- tissue_ref230_subset %>%
  dplyr::select(grep("X230", colnames(tissue_ref230_subset), value = T))
col_data230 <- col_data[col_data$isolate=="230",]
# only 220 isolate
i220_ref230 <- tissue_ref230_subset %>%
  dplyr::select(grep("X220", colnames(tissue_ref230_subset), value = T))
