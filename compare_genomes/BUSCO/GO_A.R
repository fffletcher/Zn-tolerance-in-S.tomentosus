...
keep_prefix <- function(infile, outfile, prefix_regex = "^jgi\\|Suito230_") {
...
GENE2GO_FILE <- "annotations/isolateA.gene2go.tsv"  # cleaned two-column file
FOREGROUND_FILES <- c(
  "results/sets/core/isolateA.core.genes.txt",
  "results/sets/singleton/isolateA.singleton.genes.txt"
)
BACKGROUND_FILE <- NA  # set to a path to override default background; e.g., "sets/isolateA_universe.txt"

OUTDIR <- "results/go_enrichment/isolateA"
