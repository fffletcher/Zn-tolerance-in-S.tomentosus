...
# path to OrthoFinder output Orthogroups.tsv made in 01_orthofinder.sh (tab-delimited; columns: Orthogroup, isolateA, isolateB, isolateC)
ORTHOGROUPS_TSV="../results/orthofinder/input/OrthoFinder/Results_Dec15/Orthogroups/Orthogroups.tsv"

# path to categories table made in 02_orthogroups.sh (columns should be: Orthogroup, isolateA, isolateB, isolateC, Category)
...
# normalize line endings to prevent errors CRLF (Windows) to LF
...
# read Orthogroups.tsv and store gene string names for each isolate
FNR == NR {
  if (NR == 1) {
    # Header validation
    idxA = idxB = idxC = 0
...
# read orthogroups_categories.tsv and make set files
FNR == 1 {
  # Header mapping
...

{
  ...
  # Pull gene strings written above
