...
GENE2GO_DIR="../annotations"           # where 04_gene_2GO.sh wrote gene2go files
SETS_DIR="../results/sets"          # where 05_make_sets.sh wrote sets
LOG_DIR="../logs"           # write coverage summary here

# ----------------------------- Helper: summarize one isolate ----------------------
summarize_isolate() {
  local iso="$1"
  ...
  if [[ ! -s "$gene2go" ]]; then
    ...
    # still write zeros for transparency
    for cat in core singleton; do
