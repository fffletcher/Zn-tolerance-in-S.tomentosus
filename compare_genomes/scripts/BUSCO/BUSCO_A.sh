...
# Simple BUSCO script: run in directory with your input file
# Requirements: Singularity installed, internet for lineage download
...
singularity exec \
  ...
  "${CONTAINER}" \
  busco \
    --in "/data/isolateA/genome.fa" \
    --mode "${MODE}" \
    --lineage "${LINEAGE}" \
    --out "isolateA" \
    --out_path "/results/busco/isolateA" \
    --cpu "${CPUS}"
